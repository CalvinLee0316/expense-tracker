...
function App() {
  ...
  useEffect(()=>{
    ...
    getYearsTransactions().then(ts => {
      ...
      console.log(ts)
      ts.forEach(transaction => {
        const date = parseInt(transaction.date.substring(0,7).replace("-", ""));
        console.log(date)
        if(lineData[date]){
  ...
  async function getYearsTransactions() {
    ...
    const url = process.env.REACT_APP_API_URL + "/getTransactions/range/"+month;
  ...
  async function getTransactions(f, m) {
    ...
    if(f === "All") { 
      ...
      if(m === "") {
        url = process.env.REACT_APP_API_URL + "/getTransactions/category/:" + f.replace(" ", "&");
      } else {
        url = process.env.REACT_APP_API_URL + "/getTransactions/:" + f.replace(" ", "&") + "/" + m;
      }
    }
    console.log(url)
    const res = await fetch(url);
  ...
  function handleDelete(index, e) {
    const shouldRemove = window.confirm("Are you sure you want to delete this transaction?");
    if (shouldRemove) {
      const url = process.env.REACT_APP_API_URL + "/deleteTransaction/" + transactions[index]._id;
      fetch(url, {
        method: "DELETE",
      }).then((response) =>
        response.json().then((json) => {
  ...
  return (
    <main>
      <div class="mainTracker">
        ...
        <div class="filter">
          <select id="filter" name="filter" onChange={ev=>setFilter(ev.target.value)}>
          ...
          <input type="month" id="month" value={month} onChange={ev=>setMonth(ev.target.value)} name="month"></input>
        </div>

        <div class="transactions">
          {transactions.length>0 && transactions.sort(compare).map((transaction, index) => (
            ...
            <div class="right">
              <button class="delete-btn" onClick={e=>handleDelete(index, e)}>X</button>
            </div>
        </div>
          ))}
        </div>
      ...
      <div class="analytics">
        <h1>Analytics</h1>
        <div class="pie">
          <input type="month" class="" id="analyticsMonth" value={analyticsMonth} onChange={ev=>{
              if(ev.target.value === ""){
                setAnalyticsMonth(getCurrentYearMonth())
              } else {
                setAnalyticsMonth(ev.target.value)
              }
            }} name="analyticsMonth"></input>
